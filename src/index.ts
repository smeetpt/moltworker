 ...
 * - ANTHROPIC_API_KEY or GEMINI_API_KEY: Your model provider API key
 *
 * Optional secrets:
...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  if (!hasCloudflareGateway && !hasLegacyGateway && !hasAnthropicKey && !hasGeminiKey && !hasOpenAIKey) {
    missing.push(
      'ANTHROPIC_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, or CLOUDFLARE_AI_GATEWAY_API_KEY + CF_AI_GATEWAY_ACCOUNT_ID + CF_AI_GATEWAY_GATEWAY_ID',
    );
  }
...
app.all('*', async (c) => {
  ...
  try {
    ...
    if (!c.env.ANTHROPIC_API_KEY && !c.env.GEMINI_API_KEY && !c.env.OPENAI_API_KEY) {
      hint =
        'No direct provider key is set. Run: wrangler secret put ANTHROPIC_API_KEY or wrangler secret put GEMINI_API_KEY';
